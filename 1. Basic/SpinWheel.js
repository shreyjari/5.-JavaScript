...
const SufficientBalance = (balance, bet, lines) => {
  ...
  while (true) {
    if (balance == 0 || balance < bet * lines) {
      console.log(
        "You have insufficient balance to continue playing. Kindly deposit more money or exit the game.",
      );
      deposit();
    } else if (balance > 0) {
      ...
      if (PlayAgain.toLowerCase() == "y") {
        ...
        console.table({
          "Bet Amount is": "$" + Finalbet * 3,
          "Remaining balance is": "$" + (balance - Finalbet * 3),
        });
